...
def bridge_planning_deletion_preview_payload(payload: object) -> tuple[dict[str, object], int]:
    try:
        ...
        if status != 200:
            return _planning_failure("conflict" if status == 409 else "invalid" if status == 400 else "not_found" if status == 404 else "unavailable", status)
        if not isinstance(source, dict) or set(source) != {"schema_version", "target_kind", "target_id", "confirmation_id", "affected", "has_active_runs", "retained_context_versions", "retained_input_versions", "retained_deliverable_versions", "retained_deliverable_reviews", "retained_plan_versions"}:
            raise BridgeConversationProjectionError("planning_deletion_invalid")
        ...
        if type(source.get("retained_deliverable_versions")) is not int or not 0 <= source["retained_deliverable_versions"] <= 256:
            raise BridgeConversationProjectionError("planning_deletion_invalid")
        if type(source.get("retained_deliverable_reviews")) is not int or not 0 <= source["retained_deliverable_reviews"] <= 256:
            raise BridgeConversationProjectionError("planning_deletion_invalid")
        if type(source.get("retained_plan_versions")) is not int or not 0 <= source["retained_plan_versions"] <= 256:
            raise BridgeConversationProjectionError("planning_deletion_invalid")
        ...
        return {
            ...
            "retained_deliverable_reviews": source["retained_deliverable_reviews"],
            "retained_plan_versions": source["retained_plan_versions"],
        }, 200
    except BridgeConversationProjectionError:
        return _planning_failure("error", 500)
    except Exception:
...
class BridgeRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
        ...
        if parsed.path.startswith(PROJECT_CONTEXT_ROOT):
            ...
            return
        if parsed.path.startswith(PROJECT_DELIVERABLE_ROOT):
            from project_deliverables_http import READ_OPERATIONS, dispatch_project_deliverables
            from server import DATA_DIR
            operation = parsed.path[len(PROJECT_DELIVERABLE_ROOT):]
            if operation not in READ_OPERATIONS or len(parsed.query) > 1024:
                self._send_json({'error': 'bridge_route_not_found'}, 404)
                return
            try:
    ...
    def do_POST(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
        ...
        if parsed.path.startswith(PROJECT_CONTEXT_ROOT) and not parsed.query:
            ...
            self._send_json(payload, status)
            return
        if parsed.path.startswith(PROJECT_DELIVERABLE_ROOT) and not parsed.query:
            from project_deliverables_http import WRITE_OPERATIONS, MAX_ACTION_BYTES, dispatch_project_deliverables
            from server import DATA_DIR
            operation = parsed.path[len(PROJECT_DELIVERABLE_ROOT):]
            if operation not in WRITE_OPERATIONS:
                self._send_json({'error': 'bridge_route_not_found'}, 404)
