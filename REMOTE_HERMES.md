...
| Durable Kanban delegation and follow-up | `hermes_kanban.py` uses fixed shell-free `hermes kanban` operations with task/run read-back | Current Hermes protects dashboard-plugin HTTP routes with its separate process/session dashboard token and WebSocket authorization, but the API server does not advertise Kanban through its bearer-authenticated capability document | The dashboard browser/session token is not an approved stable remote server-to-server boundary | Preserve exact preview/confirmation, mutation locks, in-flight reservation, live task/run binding, idempotency, and operation-specific read-back | **Required**; upstream API-server capability blocker |
...
| Console input attachments and Context Packs | `agent_console_attachments.py`, `agent_console_artifacts.py`, and Mentat-owned Context Pack resolution stage local snapshots | Hermes Runs currently accepts text; Hermes draft PR #68202 proposes bounded data-URL images on that same stoppable Runs lifecycle. Chat/Responses support inline images but are not a substitute. No approved API accepts general uploaded files or arbitrary local paths | API-server bearer key; Mentat-owned files stay local unless explicitly transmitted through a supported bounded content type | Bind one-use Context Pack grants to the connection, pack revision, and exact snapshots; enforce text bounds and never transmit a path. Require the released, capability-advertised Runs image contract and authenticated transport verification before sending images | **Required** bounded Context Pack text implemented in 2F; draft image capability is not enabled; general files **gracefully degrade** |
...
5. bounded Context Pack text and supported image inputs; **Milestone 2F
   implements exact one-use, path-free Context Pack text. Hermes draft PR
   #68202 proposes bounded images for the stoppable Runs lifecycle, but Mentat
   keeps images blocked until it is merged, released, advertised, and verified**;
...
8. read-only profile discovery through a supported upstream capability;
9. Kanban delegation and follow-up through a supported upstream capability;
