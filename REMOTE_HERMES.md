...
Local webhook verification: 2026-08-14 against stock Hermes 0.20.1 at release
tag `v2026.8.13`
...
Agent Console now selects a binding-aware local or remote transport. Local runs
prefer the supported authenticated headless session in
`hermes_local_control.py`, with the established one-shot launch retained only
as a pre-submission compatibility fallback. Remote runs support active profiles
...
| Agent Console conversation and streaming | `hermes_transport.py` selects a binding-aware transport; `hermes_local_control.py` owns the fixed loopback headless session and exact active-turn redirect; the transport also implements the active remote profile Runs adapter | Hermes documents Chat Completions, Responses, run submission, SSE events, approvals, and session chat in the [API Server](https://github.com/NousResearch/hermes-agent/blob/main/website/docs/user-guide/features/api-server.md) and [programmatic integration guide](https://github.com/NousResearch/hermes-agent/blob/main/website/docs/developer-guide/programmatic-integration.md) | Local control uses a per-process caller-generated dashboard token on an ephemeral loopback socket; remote control uses the API-server bearer key over verified HTTPS. Both credentials remain in Mentat's server process and are never interchangeable | Bind each run to the active connection and exact runtime session; for local steering require `message.start` plus an exact `session.redirect` receipt and reject queue semantics; for remote work keep one stream through interactive waits, verify monotonic cursors and bounded replay, and never retry ambiguous submission or steering | **Required**; bounded Context Pack text and capability-gated images are supported; general Console file output is not |
