...
class CiQualityGateTests(unittest.TestCase):
    ...
    def test_package_and_browser_gates_are_fixed_and_secret_free(self):
        ...
        self.assertIn('if [ "$bootstrap_ready" != true ]; then', workflow)
        self.assertIn('if ! wait "$bootstrap_pid"; then', workflow)
        self.assertIn('tail -c 8192 "$RUNNER_TEMP/mentat-web-bootstrap.log"', workflow)
        self.assertIn("Bootstrap remained reachable after shutdown", workflow)
        self.assertLess(workflow.index('if [ "$bootstrap_ready" != true ]; then'), workflow.index('kill -INT "$bootstrap_pid"'))
        self.assertLess(workflow.index('kill -INT "$bootstrap_pid"'), workflow.index("python scripts/mentat_web_preview.py --port 8896"))
    ...
    def test_artifact_allowlists_exclude_private_and_test_content(self):
        self.assertEqual(PUBLIC_PACKAGES, {"mentat", "deploy", "deploy.caddy"})
        ...
        self.assertEqual(set(PUBLIC_DATA_FILES), {"share/mentat/public", "share/mentat/data", "share/mentat/setup"})
