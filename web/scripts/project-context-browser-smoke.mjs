...
try {
  ...
  const inboxOutput = resolve('../artifacts/project-context-editor'); await mkdir(inboxOutput, { recursive: true });
  await page.screenshot({ path: resolve(inboxOutput, `${width}-inbox.png`), fullPage: true });
  await click('Accept saved results', 'section[aria-label="Inbox item detail"]'); await click('Preview acceptance');
  await waitText('Accept these exact versions?');
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth + 1), false, 'Inbox review creates horizontal overflow');
  await click('Confirm acceptance', 'section[aria-label="Inbox item detail"]'); await waitText('Acceptance recorded.');
  await page.goto(`http://127.0.0.1:${port}/tasks`, { waitUntil: 'domcontentloaded' });
  await click('Open results'); await waitText('The current three results are accepted.');
