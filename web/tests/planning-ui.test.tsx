...
function mutationRefreshFixture() {
  ...
  globalThis.fetch = async (input, init) => {
    ...
    if (path.endsWith("/delete/preview")) return Response.json({ ...envelope, affected: { projects: 0, tasks: 1, conversations: 0, runs: 0, artifacts: 0 }, confirmation_id: "a".repeat(64), has_active_runs: false, retained_context_versions: 0, retained_input_versions: 0, retained_deliverable_versions: 0, retained_deliverable_reviews: 0, retained_plan_versions: 0, target_kind: "task", target_id: task.id });
...
test("Task deletion shows only count effects, then confirms the exact preview before refreshing authority", async () => {
  ...
  const deletionPreview = { ...envelope, affected: { artifacts: 0, conversations: 1, projects: 0, runs: 1, tasks: 2 }, confirmation_id: "a".repeat(64), has_active_runs: true, retained_context_versions: 0, retained_input_versions: 0, retained_deliverable_versions: 0, retained_deliverable_reviews: 0, retained_plan_versions: 0, target_id: task.id, target_kind: "task" as const };
...
test("Project deletion clears every local reminder schedule because a count-only cascade can span dependent Tasks", async () => {
  ...
  const deletionPreview = { ...envelope, affected: { artifacts: 0, conversations: 0, projects: 1, runs: 0, tasks: 2 }, confirmation_id: "b".repeat(64), has_active_runs: false, retained_context_versions: 2, retained_input_versions: 0, retained_deliverable_versions: 2, retained_deliverable_reviews: 2, retained_plan_versions: 2, target_id: project.id, target_kind: "project" as const };
  ...
  await screen.findByText("Saved Project context (2 versions) and its files will remain in history.");
  await screen.findByText("Saved Project deliverables (2 versions) and their previews will remain in history.");
  await screen.findByText("Owner review decisions (2) will remain in history.");
  await screen.findByText("Saved Project plans (2 versions) will remain in history.");
