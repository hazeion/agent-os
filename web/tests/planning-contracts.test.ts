...
const runOncePreview = { ...envelope, action: "run_once" as const, confirmation_id: "a".repeat(64), requires_confirmation: true as const, objective: { text: "Research garage storage.\n\nOperator-requested changes for this exact next attempt:\nKeep the budget below $500.", redacted: false, truncated: false }, task: executionTask };
...
test("Task execution uses fixed bounded projections and exact run-once and review bodies", async () => {
  ...
  for (const objective of [undefined, { text: "", redacted: false, truncated: false }, { text: "x".repeat(20_001), redacted: false, truncated: false }, { text: "hidden\u202econtrol", redacted: false, truncated: false }, { text: "valid", redacted: "false", truncated: false }, { ...runOncePreview.objective, private_path: "no" }]) {
    assert.throws(() => parsePlanningRunOncePreview({ ...runOncePreview, objective }, nonAttentionTask.id, 1), PublicPlanningError);
  }
