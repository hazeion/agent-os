...
const PREVIEW_KEYS = "affected,confirmation_id,has_active_runs,retained_context_versions,retained_input_versions,runtime,schema_version,service,status,target_id,target_kind";
...
export function parsePlanningDeletionPreview(value: unknown, targetKind?: PlanningDeletionTargetKind, targetId?: string): PublicPlanningDeletionPreview {
  if (!record(value) || !keys(value, PREVIEW_KEYS) || !validEnvelope(value) || !validTarget(value.target_kind, value.target_id)
    ...
    || !Number.isSafeInteger(value.retained_input_versions) || (value.retained_input_versions as number) < 0 || (value.retained_input_versions as number) > 256) throw new PublicPlanningDeletionError("response_invalid");
