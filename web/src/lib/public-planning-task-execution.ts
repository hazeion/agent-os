...
function validTaskExecution(value: unknown): value is PublicPlanningTaskExecution["execution"] {
  if (!record(value) || !keys(value, "attempt_count,attempts,available,reason,recovery,review") || typeof value.available !== "boolean" || ![null, "unavailable", "project_inputs_unavailable"].includes(value.reason as string | null) || !Array.isArray(value.attempts) || value.attempts.length > 8 || !value.attempts.every(validExecutionAttempt) || new Set(value.attempts.map((attempt) => attempt.run_id)).size !== value.attempts.length || !Number.isSafeInteger(value.attempt_count) || value.attempt_count !== value.attempts.length || !record(value.review) || !keys(value.review, "available,run_id") || !record(value.recovery) || !keys(value.recovery, "available,run_id,run_revision") || typeof value.recovery.available !== "boolean") return false;
