...
function sameHeads(preview: DeliverableReviewPreview, opened: InboxOpen): boolean {
  if (!opened.project || preview.project_id !== opened.project.project.id || preview.project_revision !== opened.project.project.revision || preview.heads.length !== 3) return false;
...
export function OwnerInboxWorkspace() {
  ...
  async function acknowledge() {
    ...
    try {
      ...
      const updated: InboxItem = { ...detail.item, revision: result.revision, unread: false, acknowledged: true };
      setDetail((current) => current && { ...current, item: updated });
      ...
      setDetailNotice("Acknowledged. The result still needs its own review if it remains open.");
      detailHeading.current?.focus();
      void loadPage(view, null, true);
    } catch (error) {
      try { const current = await ownerInbox("open", { item_id: detail.item.id }); setDetail(current); setDetailNotice(current.item.acknowledged ? "Acknowledged. The result review remains separate." : "Acknowledgment could not be verified. " + message(error)); if (current.item.acknowledged) detailHeading.current?.focus(); void loadPage(view, null, true); }
  ...
  async function previewDecision() {
    if (!detail?.project || !action || locked.current) return;
    locked.current = true; setBusy(true); setDetailNotice("");
  ...
  async function confirmDecision() {
    if (!detail?.project || !preview || locked.current) return;
  ...
  const reviewable = detailState === "ready" && detail?.item.state === "needs_review" && !!detail.project && !!detail.review;
  return <section aria-label="Owner Inbox" className="owner-inbox-workspace">
    ...
    <div className="owner-inbox-layout"><section aria-label="Inbox items"><h2 ref={listHeading} tabIndex={-1}>Inbox items</h2>{page?.items.length ? <ul className="owner-inbox-list">{page.items.map((item) => <li key={item.id}><button aria-current={detail?.item.id === item.id ? "true" : undefined} data-inbox-item-id={item.id} disabled={busy} onClick={() => { originatingItem.current = item.id; void openItem(item.id); }} ref={(element) => { if (element) itemButtons.current.set(item.id, element); else itemButtons.current.delete(item.id); }} type="button"><strong>{item.title}</strong><span>{item.state === "needs_review" ? "Needs review" : item.state === "activation_required" ? "Project not active" : item.state === "resolved" ? "Resolved" : "Stale"}{item.unread ? " · Unread" : ""}{item.acknowledged ? " · Acknowledged" : ""}</span></button></li>)}</ul> : listState === "ready" ? <p>No items in this view.</p> : null}{page?.next_cursor ? <button disabled={busy} onClick={() => void loadPage(view, page.next_cursor)} type="button">Load more</button> : null}</section>
    <section aria-label="Inbox item detail" className="owner-inbox-detail"><h2 ref={detailHeading} tabIndex={-1}>Review Project results</h2>{detailState !== "idle" ? <button disabled={busy} onClick={back} type="button">Back to Inbox items</button> : null}<p aria-live="polite" role="status">{detailNotice || (detailState === "loading" ? "Opening exact review…" : "")}</p>
      {detail?.item ? <><h3>{detail.item.title}</h3><p>{detail.item.state === "needs_review" ? "These exact saved results need your decision." : detail.item.state === "activation_required" ? "This Project must be active before its results can be reviewed. Its saved versions are below." : detail.item.state === "resolved" ? "This review item is finished. The exact saved versions are below." : "This item no longer names current Project results. Its saved versions are below."}</p>{detail.item.state === "resolved" ? <a href="/tasks">Open Projects & Tasks</a> : null}<div className="project-context-actions"><button disabled={busy} onClick={() => void refreshItem()} type="button">Refresh item</button><button disabled={busy || detail.item.acknowledged} onClick={() => void acknowledge()} type="button">Acknowledge</button></div></> : null}
