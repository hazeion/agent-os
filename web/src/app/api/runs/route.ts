import { BridgeRunsError, fetchBridgeRuns, type PublicBridgeRuns } from "@/lib/bridge-runs";
...
const GET_RULE = route("GET", "/api/runs");
...
export function createRunsGetHandler({
  fetchRuns = fetchBridgeRuns,
  gatewayPort = process.env.PORT,
}: Readonly<{
  fetchRuns?: () => Promise<PublicBridgeRuns>;
  gatewayPort?: string;
}> = {}) {
  return withGatewayRoute<boolean>(GET_RULE, {
    ...
    handler: async ({ value: hasQuery }) => {
      ...
      try { return Response.json(await fetchRuns(), { headers }); }
      catch (error) {
...
const get = createRunsGetHandler();
