...
const POST_RULE = GATEWAY_ROUTE_MANIFEST.find((rule) => rule.method === "POST" && rule.path === "/api/runs/[runId]/stop/preview");
if (!POST_RULE) throw new Error("missing gateway manifest rule for POST /api/runs/[runId]/stop/preview");

type RouteContext = { params: Promise<{ runId: string }> };
type StopPreviewInput = { runId: string } | null;

const post = withGatewayRoute<StopPreviewInput, RouteContext>(POST_RULE, {
  handler: async ({ value }) => {
    if (!value) return result(400, "error");
    try { return Response.json(await fetchBridgeRunStopPreview(value.runId), { headers }); }
  ...
  validator: {
    async validate(request, _approved, context) {
      if (!await hasExactEmptyJsonBody(request)) return null;
