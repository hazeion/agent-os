import { BridgeRunMessageError, previewBridgeRunMessage } from "@/lib/bridge-run-message";
import { readMessagePreview } from "@/lib/exact-json-body";
...
const POST_RULE = GATEWAY_ROUTE_MANIFEST.find((rule) => rule.method === "POST" && rule.path === "/api/runs/[runId]/message/preview");
if (!POST_RULE) throw new Error("missing gateway manifest rule for POST /api/runs/[runId]/message/preview");

type RouteContext = { params: Promise<{ runId: string }> };
type MessagePreviewInput = { runId: string; text: string } | null;

const post = withGatewayRoute<MessagePreviewInput, RouteContext>(POST_RULE, {
  handler: async ({ value }) => {
    if (!value) return result(400, "error");
    try { return Response.json(await previewBridgeRunMessage(value.runId, value.text), { headers }); }
  ...
  validator: {
    async validate(request, _approved, context) {
      const text = await readMessagePreview(request);
      if (!text) return null;
